...
router.get("/load", async (req, res, next) => {
  ...
  sql =
    sql + " ORDER BY parent_id ASC,menu_id ASC limit " + start + "," + length;
  ...
  for (var i in result) {
    backResult.data.push({
      ...
      created_at: result[i].created_at
        ? moment(result[i].created_at).format("YYYY-MM-DD HH:mm:ss")
        : "",
      modified_at:
        result[i].modified_at != "0000-00-00 00:00:00"
          ? moment(result[i].modified_at).format("YYYY-MM-DD HH:mm:ss")
          : "",
    });
  }
...
router.get("/tree", async (req, res, next) => {
  ...
  var tree = query.tree;
  const system_code = query.system_code;
  const menu_type = query.menu_type;
  const menu_id = query.menu_id;
  var result = {
    error: 0,
    data: {},
  };
  try {
    let sql =
      "select * from bs_menu where is_del=0 and system_code is not null";
    if (!tree && system_code) {
      sql = sql + ` and  system_code=${system_code}`;
    }
    // edit
    if (tree == 0) {
    ...
    if (tree == 1) {
      sql =
        sql + ` and  system_code=${system_code} and menu_type<${menu_type} `;
    }
    // add
    if (tree == 2) {
...
router.get("/save", async (req, res, next) => {
  ...
  try {
    ...
    const e_system_code = req.query.e_system_code;
    const e_menu_order = req.query.e_menu_order || 0;
    ...
    if (e_system_code == "" || e_system_code.trim() == "") {
      result.msg = "系统ID不能为空";
    }
    ...
    if (result.msg != "") {
      ...
      if (e_menu_flag) {
        sql =
          "select * from bs_menu where menu_flag=? and is_del=0 and type = 0";
        var menus = await mysql.query(sql, e_menu_flag);
        if (
          ((!e_id || e_id == 0) && menus.length > 0) ||
          (e_id && e_id != 0 && menus.length > 0 && e_id != menus[0]["menu_id"])
        ) {
          result.error = 1;
          result.msg = "菜单唯一值已经存在！";
          res.status(200).json(result);
          return;
        }
      }
      ...
      if (e_id && e_id != 0) {
        sql =
          "update bs_menu set menu_name=?,system_code=?,parent_id=?,menu_url=?,menu_icon=?, menu_flag=?, menu_type=?, menu_order=?, modified_id=?, modified_at=? where menu_id=?";
        var params = [
          e_menu_name,
          e_system_code,
          e_parent_id,
        ...
        await common.saveOperateLog(
          req,
          "更新菜单：" + e_menu_name + ";ID: " + e_id
        );
      } else {
        ...
        //     result.msg = "菜单名已经存在！";
        // } else {
        //     sql = "insert bs_menu(menu_name, system_code,parent_id,menu_url,menu_icon,menu_flag,menu_type,menu_order,creator_id) values (?,?,?,?,?,?,?,?,?)";
        //     ret = await mysql.query(sql, [e_menu_name, e_system_code,e_parent_id, e_menu_url, e_menu_icon,e_menu_flag,e_type, e_menu_order,user.id]);
        //     await common.saveOperateLog(req, "新增菜单：" + e_menu_name);
        // }
        sql =
          "insert bs_menu(menu_name, system_code,parent_id,menu_url,menu_icon,menu_flag,menu_type,menu_order,creator_id) values (?,?,?,?,?,?,?,?,?)";
        ret = await mysql.query(sql, [
          e_menu_name,
          e_system_code,
          e_parent_id,
          e_menu_url,
    ...
    log.error("save menu ret:", JSON.stringify(e));
    result.error = 1;
...
router.delete("/delete", async (req, res, next) => {
  ...
  if (ids && ids.trim() != "") {
    ...
    try {
      ...
      var sql3 =
        "update bs_menu set is_del=1, modified_at=?, modified_id=? where menu_id in (";
